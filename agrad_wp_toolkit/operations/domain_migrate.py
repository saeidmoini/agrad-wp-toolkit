...
def run_domain_migration() -> None:
    ...
    if not wp_cli.ensure_wp_cli():
        return
    _run_migration(site_path, old_domain, new_domain, user)


def _run_migration(site_path: Path, old_domain: str, new_domain: str, run_as: str) -> None:
    replacements = [
    ...
    wp_cli._run_wp(["option", "update", "home", f"https://{new_domain}"], site_path, run_as=run_as)  # type: ignore[attr-defined]
    wp_cli._run_wp(["option", "update", "siteurl", f"https://{new_domain}"], site_path, run_as=run_as)  # type: ignore[attr-defined]
    ...
    wp_cli._run_wp(["cache", "flush"], site_path, run_as=run_as)  # type: ignore[attr-defined]
    wp_cli._run_wp(["rewrite", "flush", "--hard"], site_path, run_as=run_as)  # type: ignore[attr-defined]
    wp_cli._run_wp(["elementor", "flush-css"], site_path, run_as=run_as)  # type: ignore[attr-defined]
